...
def make_arg_head_vocab(train_file, dev_file, test_file, arghead_vocab_path, arghead2id_path, id2arghead_path, freq_lower_bound=0, quiet=False, use_lower_bound = False):
    ...
    for sentence in tqdm(origin_data):
        for line in sentence:
            for i in range(len(line)-14):
                if line[14+i] != '_':
                    argument_data.append(sentence[int(line[8])-1][2])
    ...
    if not quiet:
        print('\targhead vocab size:{}'.format(len(argument_vocab)))

    if not quiet:
        print('\tdump vocab at:{}'.format(arghead_vocab_path))

    with open(arghead_vocab_path, 'w') as f:
        f.write('\n'.join(argument_vocab))

    with open(arghead2id_path,'wb') as f:
        pickle.dump(arg_to_id,f)

    with open(id2arghead_path,'wb') as f:
        pickle.dump(id_to_arg,f)
