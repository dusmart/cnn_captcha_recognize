    ...
    for verb in tqdm(groundtruths.keys()):
        ...
        for truth_label in groundtruth.keys():
            for predict_label in predict.keys():
                ...
                if truth_label in groundtruth_same:
                    groundtruth_same[truth_label] = max(num, groundtruth_same[truth_label])
                else:
                    groundtruth_same[truth_label] = num
                if predict_label in predict_same:
                    predict_same[predict_label] = max(num, predict_same[predict_label])
                else:
                    predict_same[predict_label] = num
        precisions[verb] = 0
        pre_base = 0
        collocations[verb] = 0
        coll_base = 0
...
def main():
    ...
    pre, coll, pres, colls = eval_f1(truths, predicts)
